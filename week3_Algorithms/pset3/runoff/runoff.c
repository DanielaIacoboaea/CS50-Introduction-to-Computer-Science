...
bool is_tie(int min)
{
    int i, temp, random_vote;
    temp = 0;
    while (candidates[temp].eliminated == true)
    {
        temp++;
    }

    random_vote = candidates[temp].votes;

    for (i = 0; i < candidate_count; i++)
    {
        if((candidates[i].votes != random_vote) && candidates[i].eliminated == false)
        {
            return false;
        }
    }
